...
enum syscalls {READ = 0, WRITE, RECTANGLE, CLEAR, COORDS, SCREENINFO, FONTINFO, GETTIME, SETSCALE, SETCTRLRACTION, SLEEP, PLAYSOUND};
...
uint64_t syscallDispatcher(uint64_t rdi, uint64_t rsi, uint64_t rdx, uint64_t rcx, uint64_t r8, uint64_t rax) {
    ...
    switch (rax) {
        ...
        case SETCTRLRACTION: return sys_setCtrlRAction((void (*)(void))rdi);
